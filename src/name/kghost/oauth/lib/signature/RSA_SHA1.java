...
public class RSA_SHA1 extends OAuthSignatureMethod {
	...
	protected void initialize(String name, OAuthAccessor accessor)
		...
		try {
			Object privateKeyObject = accessor.consumer
			...
			if (publicKeyObject != null) {
				...
				Object certObject = accessor.consumer
						.getProperty(X509_CERTIFICATE);
				if (certObject != null) {
	...
	private PrivateKey getPrivateKeyFromPem(String pem)
		...
		} else if (PEMReader.PRIVATE_PKCS8_MARKER.equals(reader
				.getBeginMarker())) {
	...
	private PrivateKey loadPrivateKey(Object privateKeyObject)
		...
		if (privateKeyObject instanceof PrivateKey) {
			priKey = (PrivateKey) privateKeyObject;
		} else if (privateKeyObject instanceof String) {
			try {
				// PEM Reader's native string constructor is for filename.
				priKey = getPrivateKeyFromPem((String) privateKeyObject);
			} catch (IOException e) {
				// Check if it's PEM with markers stripped
				priKey = getPrivateKeyFromDer(decodeBase64((String) privateKeyObject));
			}
		} else if (privateKeyObject instanceof byte[]) {
			priKey = getPrivateKeyFromDer((byte[]) privateKeyObject);
		} else {
	 ...
	 *            The object for public key or certificate
	 * @param isCert
	 *            True if this object is provided as Certificate
	...
	private PublicKey loadPublicKey(Object publicKeyObject, boolean isCert)
		...
		if (publicKeyObject instanceof PublicKey) {
			pubKey = (PublicKey) publicKeyObject;
		} else if (publicKeyObject instanceof X509Certificate) {
			pubKey = ((X509Certificate) publicKeyObject).getPublicKey();
		} else if (publicKeyObject instanceof String) {
			try {
				pubKey = getPublicKeyFromPem((String) publicKeyObject);
			} catch (IOException e) {
				// Check if it's marker-stripped PEM for public key
				if (isCert)
					throw e;
				pubKey = getPublicKeyFromDer(decodeBase64((String) publicKeyObject));
			}
		} else if (publicKeyObject instanceof byte[]) {
			if (isCert)
				pubKey = getPublicKeyFromDerCert((byte[]) publicKeyObject);
			else
				pubKey = getPublicKeyFromDer((byte[]) publicKeyObject);
		} else {
			String source;
			if (isCert)
			...
			throw new IllegalArgumentException(
					"Public key or certificate set through " + source
							+ " must be of "
